...
void AKAZE::Allocate_Memory_Evolution() {
  ...
  cuda_memory =
      AllocBuffers(evolution_[0].Lt.cols, evolution_[0].Lt.rows,
                   options_.ncudaimages, options_.omax, options_.maxkeypoints,
                   cuda_buffers, cuda_points, cuda_images);
}
...
int AKAZE::Create_Nonlinear_Scale_Space(const cv::Mat& img) {
  ...
  gaussian_2D_convolution(evolution_[0].Lt, evolution_[0].Lt, 0, 0,
                          options_.soffset);
  ...
  for (size_t i = 1; i < evolution_.size(); i++) {
    ...
    switch (options_.diffusivity) {
      ...
      case CHARBONNIER:
        ...
        break;
      default:
        cerr << "Diffusivity: " << options_.diffusivity << " is not supported"
             << endl;
    }
...
void AKAZE::Compute_Determinant_Hessian_Response() {
  ...
  for (size_t i = 0; i < evolution_.size(); i++) {
    if (options_.verbosity == true)
      cout << "Computing detector response. Determinant of Hessian. Evolution "
              "time: " << evolution_[i].etime << endl;
    ...
    for (int ix = 1; ix < evolution_[i].Ldet.rows - 1; ix++) {
      ...
      for (int jx = 1; jx < evolution_[i].Ldet.cols - 1; jx++) {
        ...
        if (value > options_.dthreshold && value >= options_.min_dthreshold &&
            ...
            value > ldet_m[jx - 1] && value > ldet_m[jx] &&
            value > ldet_m[jx + 1] && value > ldet_p[jx - 1] &&
            value > ldet_p[jx] && value > ldet_p[jx + 1]) {
          is_extremum = true;
          ...
          for (size_t ik = 0; ik < kpts_aux.size(); ik++) {
            if ((point.class_id - 1) == kpts_aux[ik].class_id ||
              ...
              dist = (point.pt.x * ratio - kpts_aux[ik].pt.x) *
                         (point.pt.x * ratio - kpts_aux[ik].pt.x) +
                     (point.pt.y * ratio - kpts_aux[ik].pt.y) *
                         (point.pt.y * ratio - kpts_aux[ik].pt.y);
          ...
          if (is_extremum == true) {
            ...
            if (left_x < 0 || right_x >= evolution_[i].Ldet.cols || up_y < 0 ||
                down_y >= evolution_[i].Ldet.rows) {
              is_out = true;
            }
  ...
  for (size_t i = 0; i < kpts_aux.size(); i++) {
    ...
    for (size_t j = i + 1; j < kpts_aux.size(); j++) {
      ...
      if ((point.class_id + 1) == kpts_aux[j].class_id) {
        dist =
            (point.pt.x - kpts_aux[j].pt.x) * (point.pt.x - kpts_aux[j].pt.x) +
    ...
    if (is_repeated == false) kpts.push_back(point);
  }

  // for (unsigned int i=0;i<kpts.size();i++)
  //  printf("XXX %02d %03d %03d %f *\n", kpts[i].class_id, (int)kpts[i].pt.x,
  //  (int)kpts[i].pt.y, kpts[i].response);
...
#pragma omp parallel for
      ...
      for (int i = 0; i < (int)(kpts.size()); i++) {
        if (options_.descriptor_size == 0)
          ...
          Get_Upright_MLDB_Descriptor_Subset(kpts[i],
                                             desc.ptr<unsigned char>(i));
      }
    } break;
    case MLDB: {
#ifdef _OPENMP
...
void AKAZE::Compute_Main_Orientation(cv::KeyPoint& kpt) const {
  ...
  for (ang1 = 0; ang1 < 2.0 * CV_PI; ang1 += 0.15f) {
    ang2 = (ang1 + CV_PI / 3.0f > 2.0 * CV_PI ? ang1 - 5.0f * CV_PI / 3.0f
                                              : ang1 + CV_PI / 3.0f);
    sumX = sumY = 0.f;
  ...
  // kpt.angle/CV_PI*180.0f);
}

/* ************************************************************************* */
void AKAZE::Get_SURF_Descriptor_Upright_64(const cv::KeyPoint& kpt,
                                           float* desc) const {
...
/* ************************************************************************* */
void AKAZE::Get_MSURF_Upright_Descriptor_64(const cv::KeyPoint& kpt,
                                            float* desc) const {
...
/* ************************************************************************* */
void AKAZE::Get_MSURF_Descriptor_64(const cv::KeyPoint& kpt,
                                    float* desc) const {
  float dx = 0.0, dy = 0.0, mdx = 0.0, mdy = 0.0, gauss_s1 = 0.0,
        gauss_s2 = 0.0;
  float rx = 0.0, ry = 0.0, rrx = 0.0, rry = 0.0, len = 0.0, xf = 0.0, yf = 0.0,
...
/* ************************************************************************* */
void AKAZE::Get_MLDB_Full_Descriptor(const cv::KeyPoint& kpt,
...
/* ************************************************************************* */
void AKAZE::Get_Upright_MLDB_Full_Descriptor(const cv::KeyPoint& kpt,
                                             unsigned char* desc) const {
  const int max_channels = 3;
  CV_Assert(options_.descriptor_channels <= max_channels);
  float values[16 * max_channels];
        ...
        for (int l = j; l < j + sample_step; l++) {
          ...
          if (nr_channels > 1) {
            ...
            if (nr_channels == 2) {
              ...
              // X\n", l+pattern_size, k+pattern_size, x1, y1, ri, rx, ry, rrx,
              // rry);
              dx += rrx;
              dy += rry;
            }
...
/* ************************************************************************* */
void AKAZE::Get_MLDB_Descriptor_Subset(const cv::KeyPoint& kpt,
                                       unsigned char* desc) {
  ...
  cv::Mat values =
      cv::Mat_<float>::zeros((4 + 9 + 16) * options_.descriptor_channels, 1);
...
/* ************************************************************************* */
void AKAZE::Get_Upright_MLDB_Descriptor_Subset(const cv::KeyPoint& kpt,
                                               unsigned char* desc) {
...
/* ************************************************************************* */
void libAKAZE::generateDescriptorSubsample(cv::Mat& sampleList,
